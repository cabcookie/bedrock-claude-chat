...
import { MessageModel, ModelInvokeBody, SupportedModelArns, SupportedModels } from "../@types/schemas";
...
const createInvokeCommand = (modelId: SupportedModelArns, prompt: string) => new InvokeModelCommand({
  modelId,
...
const extractOutputText = (model: SupportedModels, response: InvokeModelResponse): string => {
  if (!['claude-instant-v1', 'claude-v2'].includes(model))
    throw new NotImplementedError(`Model '${model}' is not implemented`);
